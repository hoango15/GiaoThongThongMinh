...
class MainApp(tk.Tk, BaseWindow):
    ...
    def create_widgets(self):
        ...
        tk.Label(
            self,
            text="ℹ️ Lưu ý: Các module xử lý ảnh / DB cần đặt cùng thư mục.",
            font=("Arial", 10),
            bg="#f2f6fa",
            fg="gray"
        ).pack(pady=10)
...
def resize_keep_aspect(image, base_width):
...
class SearchWindow(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        tk.Label(frame, text="Nhập biển số:", bg="#f0f4f7", font=("Arial", 11, "bold")).grid(row=0, column=0, sticky="w")
        self.plate_var = tk.StringVar()
        tk.Entry(frame, textvariable=self.plate_var, width=30, font=("Arial", 11)).grid(row=0, column=1, sticky="w", padx=5)
        ...
        # Thông tin xe
        info_frame = tk.LabelFrame(self, text="Thông tin xe", padx=10, pady=10, bg="#ffffff",
                                   font=("Arial", 11, "bold"))
        info_frame.pack(fill="x", padx=10, pady=10)

        self.info_text = tk.Label(info_frame, text="Kết quả sẽ hiển thị ở đây...", anchor="w", justify="left",
        ...
        self.vehicle_img_label = tk.Label(imgs, text="Ảnh xe", bg="#cccccc", width=60, height=15)
        self.vehicle_img_label.grid(row=0, column=0, padx=8)

        self.plate_img_label = tk.Label(imgs, text="Ảnh biển số", bg="#cccccc", width=60, height=8)
        self.plate_img_label.grid(row=0, column=1, padx=8)
    ...
    def search(self):
        ...
        try:
            row = find_vehicle_by_plate(plate)   # Hàm này bạn đã có trong database.py
        except Exception as e:
            traceback.print_exc()
...
if vimg_path and os.path.exists(vimg_path):
    try:
        vimg = Image.open(vimg_path)   # Giữ nguyên kích thước ảnh gốc
        self.vehicle_img_label.imgtk = ImageTk.PhotoImage(vimg)
        self.vehicle_img_label.config(image=self.vehicle_img_label.imgtk, text="")
    except Exception as e:
        print("Lỗi ảnh xe:", e)
...
if pimg_path and os.path.exists(pimg_path):
    try:
        pimg = Image.open(pimg_path)   # Giữ nguyên kích thước ảnh gốc
        self.plate_img_label.imgtk = ImageTk.PhotoImage(pimg)
        self.plate_img_label.config(image=self.plate_img_label.imgtk, text="")
    except Exception as e:
        ...
        if pimg_path and os.path.exists(pimg_path):
            try:
                pimg = resize_keep_aspect(Image.open(pimg_path), )
                self.plate_img_label.imgtk = ImageTk.PhotoImage(pimg)
                self.plate_img_label.config(image=self.plate_img_label.imgtk, text="")
            except Exception as e:
                print("Lỗi ảnh biển số:", e)
