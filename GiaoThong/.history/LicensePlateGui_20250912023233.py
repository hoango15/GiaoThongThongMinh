...
from Database import get_connection, get_current_count, insert_vehicle
...
# ==================
# chỗ này import từ module khác của bạn
# ==================
# from Database import insert_vehicle, get_current_count, get_connection
# from Processing import recognize_license_plate_from_camera, process_video_from_file, add_missing_data_main, visualize_main
# from BaseWindow import BaseWindow

class EntryWindow(tk.Toplevel):  # bạn có thể thêm BaseWindow nếu cần
    def __init__(self, master):
        tk.Toplevel.__init__(self, master)
        # BaseWindow.__init__(self, master)  # nếu có class BaseWindow thì giữ lại
    ...
    def create_widgets(self):
        ...
        # hiển thị số lượng xe hiện tại
        tk.Label(left, text="🚘 Số lượng xe hiện tại:", font=("Arial", 10, "bold")).pack(anchor="w", pady=(10,2))
        self.current_count_var = tk.StringVar(value="Đang tải...")
        self.current_count_label = tk.Label(left, textvariable=self.current_count_var, font=("Arial", 12), fg="blue")
        self.current_count_label.pack(anchor="w")

        # chọn loại xe
        tk.Label(left, text="Loại xe:").pack(anchor="w", pady=(10,2))
        self.vehicle_type_var = tk.StringVar(value="Xe máy")
        self.vehicle_type_combo = ttk.Combobox(left, textvariable=self.vehicle_type_var, state="readonly")
        ...
        info_frame.place(x=10, y=420, width=960, height=250)
        ...
        status_bar.place(x=0, y=700, relwidth=1)

    def update_current_count(self):
        """Lấy số lượng xe hiện tại từ DB và cập nhật giao diện"""
        try:
            count = get_current_count()
            self.current_count_var.set(f"{count} xe")
        except Exception as e:
            self.current_count_var.set("Lỗi lấy dữ liệu")
            traceback.print_exc()
    ...
    def process_camera(self):
        ...
        try:
            recognize_license_plate_from_camera(self, mode="in")  # bỏ stop_event nếu không hỗ trợ
        except Exception as e:
            traceback.print_exc()
    ...
    def confirm_save(self):
        if not hasattr(self, "detected_plate") or not self.detected_plate:
        ...
        try:
            insert_vehicle(self.detected_plate, ticket_code, self.detected_vehicle_img, self.detected_plate_img, vehicle_type)

            # tăng số lượng xe trong DB
            conn = get_connection()
            cur = conn.cursor()
            cur.execute("UPDATE config SET current_count = current_count + 1 LIMIT 1")
            conn.commit()
