...
# ===============================
# Tạo số thứ tự file ảnh
# ===============================
def get_next_index():
    files = [f for f in os.listdir(output_folder) if f.endswith(".jpg")]
    return len(files) // 2 + 1   # mỗi lần lưu 2 file (vehicle + plate)

def save_numbered_images(frame, crop, mode="in"):
    idx = get_next_index()
    vpath = os.path.join(output_folder, f"{mode}_{idx}_vehicle.jpg")
    ppath = os.path.join(output_folder, f"{mode}_{idx}_plate.jpg")
    cv2.imwrite(vpath, frame)
    cv2.imwrite(ppath, crop)
    return vpath, ppath
                    ...
                    if corrected_plate:
                        ...
                        if time() - last_check >= 2:
                            if plate_counts.count(corrected_plate) > 5:
                                vpath, ppath = save_numbered_images(frame, crop, mode=mode)
