...
# File CSV để debug (không bắt buộc DB)
...
def save_unique_images(plate, frame, crop, mode="in"):
    """Lưu ảnh xe và biển số với tên file duy nhất."""
    ts = int(time())  # timestamp để tránh trùng
    vpath = os.path.join(output_folder, f"{plate}_{mode}_{ts}_vehicle.jpg")
    ppath = os.path.join(output_folder, f"{plate}_{mode}_{ts}_plate.jpg")

    cv2.imwrite(vpath, frame)   # ảnh toàn xe
    cv2.imwrite(ppath, crop)    # ảnh biển số

    return vpath, ppath

# ===============================
# Nhận diện từ Camera Realtime
# ===============================
def recognize_license_plate_from_camera(app, mode="in", stop_event=None):
    """
    Nhận diện realtime từ camera.
    KHÔNG lưu trực tiếp vào DB/CSV.
    Chỉ hiển thị kết quả và chờ người dùng nhấn "XÁC NHẬN".
    """
    ...
    def update():
            ...
            for box in result.boxes:
                ...
                if labels:
                    ...
                    if corrected_plate:
                        ...
                        # mỗi 2 giây kiểm tra 1 lần
                        if time() - last_check >= 2:
                            if plate_counts.count(corrected_plate) > 5:
                                # ✅ Lưu ảnh với tên riêng
                                vpath, ppath = save_unique_images(corrected_plate, frame, crop, mode)
        ...
        # ✅ gọi lại sau 30ms nếu chưa stop
