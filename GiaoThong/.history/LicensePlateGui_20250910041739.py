...
class MainApp(tk.Tk, BaseWindow):
    ...
    tk.Button(btn_frame, text="❌ Thoát", bg="#9E9E9E", fg="white", command=self.quit, **style).grid(row=1, column=1, padx=12, pady=10)
...
class SearchWindow(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        self.vehicle_img_label = tk.Label(imgs, text="Ảnh xe", bg="#cccccc", width=40, height=10)
        self.vehicle_img_label.grid(row=0, column=0, padx=8)

        self.plate_img_label = tk.Label(imgs, text="Ảnh biển số", bg="#cccccc", width=40, height=5)
        self.plate_img_label.grid(row=0, column=1, padx=8)
    ...
    def search(self):
        ...
        try:
            ...
            entry_time = row["time_in"]
            exit_time = row["time_out"]
            fee = row["fee"]
            vimg_path = row["vehicle_img_path"]
            pimg_path = row["plate_img_path"]
        except Exception:
            self.info_text.config(text=f"Tìm được: {row}")
            return

        # Thông tin text
        info = f"Biển số: {plate_number}\nMã vé: {ticket_code}\nThời gian vào: {entry_time}"
        if exit_time:
