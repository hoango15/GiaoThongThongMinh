...
def recognize_license_plate_from_camera(app):
    """Nhận diện realtime từ camera, hiển thị trong GUI Tkinter."""
                ...
                if labels:
                    ...
                    if corrected_plate:
                        ...
                        if time() - last_check >= 2:
                            # Nếu xuất hiện nhiều lần và chưa có trong CSV
                            if plate_counts.count(corrected_plate) > 5 and not is_plate_in_csv(output_csv_file, corrected_plate):
                                with open(output_csv_file, 'a', newline='', encoding='utf-8') as f:
                                    csv.writer(f).writerow(
                                        [int(cap.get(cv2.CAP_PROP_POS_FRAMES)), corrected_plate, r.tolist()]
                                    )

                                # Lưu ảnh xe & biển số
                                vpath = os.path.join(output_folder, f'vehicle_{int(time())}.jpg')
                                ppath = os.path.join(output_folder, f'plate_{int(time())}.jpg')
                                cv2.imwrite(vpath, frame)
                                cv2.imwrite(ppath, frame[r[1]:r[3], r[0]:r[2]])

                                # Cập nhật GUI
                                app.update_info(corrected_plate, None, time(), vpath, ppath)
