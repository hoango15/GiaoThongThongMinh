...
import threading, time, os, random, string, traceback
from PIL import Image, ImageTk
import cv2

# giữ nguyên các module backend
...
# ================== MAIN APP ==================
class MainApp(ctk.CTk, BaseWindow):
    def __init__(self):
    ...
    def create_widgets(self):
        title = ctk.CTkLabel(self, text="HỆ THỐNG QUẢN LÝ BÃI ĐỖ XE", font=("Arial", 20, "bold"))
        title.pack(pady=20)
        ...
        # Buttons grid
        btn1 = ctk.CTkButton(frame, text="🚗 Xử lý - Xe vào", command=self.open_entry_window, width=200, height=50)
        btn2 = ctk.CTkButton(frame, text="🚙 Xử lý - Xe ra", command=self.open_exit_window, width=200, height=50)
        btn3 = ctk.CTkButton(frame, text="🔍 Tìm xe theo biển số", command=self.open_search_window, width=200, height=50)
        btn4 = ctk.CTkButton(frame, text="🎞 Xử lý Video (batch)", command=self.open_video_process_dialog, width=200, height=50)
        ...
        note = ctk.CTkLabel(self, text="ℹ️ Đặt các module xử lý ảnh / DB cùng folder để chạy.", font=("Arial", 12))
        note.pack(pady=10)
...
# ================== ENTRY WINDOW ==================
class EntryWindow(ctk.CTkToplevel, BaseWindow):
    ...
    def create_widgets(self):
        # Left control
        left = ctk.CTkFrame(self, corner_radius=15)
        left.place(x=20, y=20, width=300, height=300)

        ctk.CTkLabel(left, text="Nguồn camera/video", font=("Arial", 14, "bold")).pack(pady=10)

        self.source_var = ctk.StringVar(value="camera")
        ctk.CTkRadioButton(left, text="Camera realtime", variable=self.source_var, value="camera").pack(anchor="w", padx=20)
        ctk.CTkRadioButton(left, text="File video", variable=self.source_var, value="video").pack(anchor="w", padx=20)

        ctk.CTkButton(left, text="▶ BẮT ĐẦU", fg_color="green", command=self.start).pack(fill="x", padx=20, pady=10)
        ctk.CTkButton(left, text="⏹ DỪNG", fg_color="red", command=self.stop).pack(fill="x", padx=20, pady=5)
        ctk.CTkButton(left, text="💾 XÁC NHẬN LƯU", fg_color="blue", command=self.confirm_save).pack(fill="x", padx=20, pady=20)

        # Video
        self.video_label = ctk.CTkLabel(self, text="Video Preview", fg_color="black")
        self.video_label.place(x=340, y=20, width=640, height=480)

        # Info
        info = ctk.CTkFrame(self, corner_radius=15)
        info.place(x=20, y=340, width=960, height=280)

        self.info_text = ctk.CTkLabel(info, text="Chưa có dữ liệu nhận diện...", anchor="w", justify="left")
        self.info_text.pack(pady=10, padx=10)

        imgs = ctk.CTkFrame(info)
        imgs.pack(pady=6)
        self.vehicle_img_label = ctk.CTkLabel(imgs, text="Ảnh xe")
        self.vehicle_img_label.grid(row=0, column=0, padx=20)
        self.plate_img_label = ctk.CTkLabel(imgs, text="Ảnh biển số")
        self.plate_img_label.grid(row=0, column=1, padx=20)

        status_bar = ctk.CTkLabel(self, textvariable=self.status_var, anchor="w", fg_color="gray20")
...
class SearchWindow(tk.Toplevel):
    ...
    def search(self):
        ...
        # Giả sử row format: (id, license_plate, ticket_code, entry_time, vehicle_img_path, plate_img_path, exit_time, cost)
        # Nếu DB của bạn khác, chỉnh lại chỉ mục
        try:
            ticket_code = row[2]
            entry_time = row[3]
